...
void loop() {
  ...
  if (packetSize) {
    ...
    // A$+A11A... problema nel trovare i caratteri identificativi (A11A11A11A11A11A11A11A11) e nel distinguerli dal numero che ci arriva (si potrebbero contare i bit siccome nota la distanza tra due misure contigue)
    ...
    for (int j = 0; j < packetSize; j = j+3) {
      for (int i = 0; i < num_beehives; i++) {
        if (((packet[j] == BEEHIVES_ID[i]) && (packet[j+3] == BEEHIVES_ID[i])) || ((packet[j] == BEEHIVES_ID[i]) && (packet[j+6] == BEEHIVES_ID[i])) || ((packet[j+3] == BEEHIVES_ID[i]) && (packet[j+6] == BEEHIVES_ID[i]))) {
