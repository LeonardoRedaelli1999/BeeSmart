...
void setup(void)
{
  ...
  // BRUTTO we could implement the check for connection to receiver
  
  // BRUTTO we could save data on a MicroSD to send them via LoRa only once a day (Solar Panel, so when light is max)
...
{ 
  ...
  // BRUTTO it could be used by interrupting the cycle using an external CLOCK
  ...
  PIR1=detect1/MaxCount*333;
  PIR2=detect2/MaxCount*449;
  ...
  delay(780000); // BRUTTO sleep for 720000 here + 60000 at the beginning of loop
}
